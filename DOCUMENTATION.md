# next-advanced-sitemap (v1.3.6) - Technical Documentation and Reference Manual
...
- **Robots.txt Builder Engine (`buildRobotsText`) (v1.3.6)**: Instant, zero-dependency helper to format RFC-compliant `robots.txt` rules, infer the root host from the first sitemap URL when `host` is omitted, expose IDE autocomplete for major crawlers via `KnownUserAgent`, flatten array-based `allow` / `disallow` directives, render multiple sequential `Sitemap:` directives when given a sitemap array (multi-sitemap injection), support explicit `Allow` sub-route overrides inside blocked directories, and omit blank values from generated rules.
...
### 5.5 Robots.txt Configuration & Formatting (v1.3.6)
...
#### `buildRobotsText(options)` (v1.3.6)
