# next-advanced-sitemap (v1.3.7) - Technical Documentation and Reference Manual
...
- **Robots.txt Builder Engine (`buildRobotsText`) (v1.3.7)**: Instant, zero-dependency helper to format RFC-compliant `robots.txt` rules, infer the root host from the first sitemap URL when `host` is omitted, expose IDE autocomplete for major crawlers via `KnownUserAgent`, flatten array-based `allow` / `disallow` directives, render multiple sequential `Sitemap:` directives when given a sitemap array (multi-sitemap injection), support explicit `Allow` sub-route overrides inside blocked directories, emit `Crawl-delay: <seconds>` for rate-limited bot crawling, and omit blank values from generated rules.
...
### 2.3 Robots.txt Builder & App Router Integration
...
### 5.5 Robots.txt Configuration & Formatting (v1.3.7)
...
const config: RobotsOptions = {
  ...
  // host is optional; it is inferred from the first sitemap origin
};
...
#### `buildRobotsText(options)` (v1.3.7)
...
## 8. Version Changelog Highlights (v1.0.0 - v1.3.7)

- **v1.3.7**: Added `Crawl-delay` support for bot rate-limiting in `buildRobotsText()`, while preserving typed `KnownUserAgent` support, automatic host discovery, multi-sitemap rendering, and explicit `Allow` sub-route override handling.
- **v1.3.6**: Added automatic array flattening for `allow` / `disallow` directives and blank-value omission in `buildRobotsText()`, preserved typed `KnownUserAgent` support and automatic host discovery, added multi-sitemap rendering (emits multiple `Sitemap:` directives when a sitemap array is provided), and added explicit `Allow` sub-route override support to permit specific paths inside blocked directories.
