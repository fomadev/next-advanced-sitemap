# next-advanced-sitemap (v1.3.9) - Technical Documentation and Reference Manual
...
- **Robots.txt Builder Engine (`buildRobotsText`) (v1.3.9)**: Instant, zero-dependency helper to format RFC-compliant `robots.txt` rules, infer the root host from the first sitemap URL when `host` is omitted, expose IDE autocomplete for major crawlers via `KnownUserAgent`, flatten array-based `allow` / `disallow` directives, render multiple sequential `Sitemap:` directives when given a sitemap array (multi-sitemap injection), support explicit `Allow` sub-route overrides inside blocked directories, emit `Crawl-delay: <seconds>` for rate-limited bot crawling, inject a generated-at metadata header comment (`# Generated by next-advanced-sitemap at ...`), and provide a safe `getRobotsTextResponse()` wrapper for `text/plain` HTTP delivery with `nosniff` protection.
...
### 5.5 Robots.txt Configuration & Formatting (v1.3.9)
...
#### `getRobotsTextResponse(options)` (v1.3.9)
...
#### `buildRobotsText(options)` (v1.3.9)
...
## 8. Version Changelog Highlights (v1.0.0 - v1.3.9)
